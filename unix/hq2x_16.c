...
HQX_API void HQX_CALLCONV hq2x_16( uint16_t * sp, uint16_t * dp, int Xres, int Yres, int XDres, int YDres )
{
    int  i, j, k;
...
//    int dpL = Xres * 2;
	int dpL = XDres;
	int centreX = (XDres - (Xres<<1)) >> 1;
	int centreY = (YDres - (Yres<<1)) >> 1;
	dp += centreY * XDres;
	dp += centreX;
	Xres = 320; // hardcoded for snes9x4p only, because this is the width of the graphics buffer, but not necessarily the width of the image
    //   +----+----+----+
    //   |    |    |    |
    //   | w1 | w2 | w3 |
    //   +----+----+----+
